...
const vec3 GRASS_ALBEDO = vec3(0.2, 0.3, 0.12);//pow(vec3(0.5, 0.9, 0.2), vec3(2.2));//rgb(27%, 46%, 7%)
const vec3 STONE_ALBEDO = vec3(0.2, 0.2, 0.2);//vec3(0.61, 0.33, 0.17);//pow(vec3(0.8, 0.6, 0.45), vec3(2.2));//rgb(82%, 71%, 54%)rgb(78%, 88%, 93%)
...
void main() {
    ...
    float stoneFactor = max(1.0 - (linstep(0.8 , 0.9, vertex.normal.y)), linstep(15.0, 20.0, vertex.altitude));
